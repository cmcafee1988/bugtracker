...
from bugsapp.forms import LoginForm, AddTicketForm, InProgressForm
...
def index_view(request):
    ...
    return render(request, 'index.html', {'title': 'Bugtracker', 'welcome': "Welcome to my version of the What's up, Doc? (Bugs, Bugs, Bugs!) assessment", 'tickets': tickets})
...
def user_detail_view(request, user_id):
    ...
    tickets_done = Ticket.objects.filter(completed_by=the_user.id)
    # breakpoint()
    return render(request, "user_detail.html", {"user": the_user, "tickets_by": tickets_by, 'tickets_assigned': tickets_assigned, 'tickets_completed': tickets_done})
...
def done_view(request, ticket_id):
    done = Ticket.objects.get(id=ticket_id)
    done.ticket_status_choices = 'D'
    done.done_by = done.assigned_to
    done.assigned_to = None
    done.save()
